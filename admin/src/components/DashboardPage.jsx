...
import { dashboardStyles as s } from '../assets/frontend/dummyStyles';
import {
  Users,
  BadgeIndianRupee,
  CalendarRange,
...
<StatCard
  ...
  value={patientCount ?? totals.totalLoginPatients}
/>
  ...
  <StatCard
    ...
    label="Total Earning"
    value={`RS ${totals.totalEarnings.toLocaleString()}`}
  />
  ...
  {filteredDoctors.length > INITIAL_COUNT && (
    <div className={s.showMoreContainer}>
      <button onClick={() => setShowAll((s) => !s)} className={s.showMoreButton + " " + s.cursorPointer}>
        {showAll ? "Showless" : `Showmore(${filteredDoctors.length - INITIAL_COUNT})`
        }
