...
export const getAppointmentByPatient = async (req, res) =>{
    try {
        ...
        console.log('resolveClerkUserId (query or req.auth.userId):', resolvedCreatedBy);
...
export const createAppointment = async (req, res) => {
   ...
   if(!clerkUserId) return res.status(402).json({
    success: false,
    message: "Authentication is requried"
   });

   if(!doctorId || !patientName || !mobile || !date || !time){
    return res.status(400).json({
        success: false,
        message: "All the field are requried"
    });
   }
   ...
   if(numericFee === null || numericFee < 0){
    return res.status(400).json({
      success: false,
      message: "Fee must be a vaild Number"
    });
   }
   ...
   if(existingBooking){
    return res.status(409).json({
      success: false,
      message: "You have already a appointment with is doctor at the selected slot"
    });
   }
   ...
   try{
    ...
    console.warn("Doctor Lookup Faild:", e?.message || e);
...
try {
  ...
  if(!session_id) return res.status(400).json({
    success: false,
    message: "session id is requried"
  });
...
return res.json({success: true, appointment: attp});
 


  } catch (err) {
    console.error("ConfrimPayment  error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
...
export const updateAppointment = async (req, res) => {
  try {
    ...
    if(!appt)return res.status(400).json({
      success: false,
...
export const cancelAppointment = async (req, res) => {
 ...
 const { id } = req.params;
    ...
    console.error("CanceledAppointment  error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
