...
import { populate } from "dotenv";
 ...
 const stripe = stripeKey ? new Stripe(stripeKey, {apiVersion: "2026-04-22"}) : null;
...
// to create serviceAppointment
export const createServiceAppoinment =  async (req, res) =>{
try {
    const body = req.body || {};
    const clerkUserId = resolveClerkUserId(req);
    if(!clerkUserId) return res.status(401).json({
        success: false,
        message: "Authentication is requried to create a service appointment"
    });
...
export const confirmServicePayment = async (req, res) => {
    try {
        ...
        if(!stripe) return res.status(500).json({
           success: false,
           message: "Stripe is Not Configred"
        });
